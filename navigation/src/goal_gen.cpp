...
void processGoal() {
  while (ros::ok()) {
    if (odom_received && map_received) {
      ...
      // double global_goal_dist =
      //     distancePoint(current_pose.world_position, current_goal.pose);
      // if (current_goal.counter == 1 && global_goal_dist < 15 &&
      //     switch_one == false) {
      //   paused = true;
      //   switch_one = true;
    ...
    //   if (current_goal.counter == 4 &&
    //       distancePoint(current_goal.pose, current_pose.world_position) <=
    //           2) {
    //     paused = false;
    //     switch_two = true;
    //
    ...
    // if (switch_two &&
    //     distancePoint(current_pose.world_position, final_pt) < 4) {
    //   paused = true;
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber global_goal_sub =
      nh.subscribe("/goal", 1, globalGoalCallback);
