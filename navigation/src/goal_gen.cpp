...
inline int gridToIndex(int x, int y, int width) {
    return y * width + x;
}
...
GridCell findClosestLaneParallel(const vector<int>& map, int width, GridCell start) {
    ...
    for (int i = 0; i < num_threads; ++i) {
        ...
        int endY = (i == num_threads - 1) ? map.size() / width : (i + 1) * sectionHeight;
...
GridCell exploreFarthestParallel(const unordered_set<GridCell, GridCellHash>& cluster, const GridCell& start, double eps) {
    const int num_threads = 4;
    vector<future<pair<GridCell, double>>> futures;

    auto exploreSection = [&](const vector<GridCell>& cells) -> pair<GridCell, double> {
    ...
    for (int i = 0; i < num_threads; ++i) {
        ...
        int endIdx = (i == num_threads - 1) ? clusterVec.size() : (i + 1) * sectionSize;
        vector<GridCell> section(clusterVec.begin() + startIdx, clusterVec.begin() + endIdx);
...
void mapCallback(const nav_msgs::OccupancyGrid::ConstPtr &msg) {
    ...
    if (!map_received) {
        ...
        ignored_cells.resize(current_map.height, vector<bool>(current_map.width, false));
    }
...
void odomCallback(const nav_msgs::Odometry::ConstPtr &msg) {
    ...
    current_pose.map_position = pointToGrid(current_pose.world_position, current_map);
...
void globalGoalCallback(const geometry_msgs::PoseStamped::ConstPtr &msg) {
    current_goal = {{msg->pose.position.x, msg->pose.position.y}, static_cast<int>(msg->pose.position.z)};
}
...
void processGoal() {
    while (ros::ok()) {
        if (odom_received && map_received) {
            ...
            // double global_goal_dist = distancePoint(current_pose.world_position, current_goal.pose);
            // if (current_goal.counter == 1 && global_goal_dist < 15 && switch_one == false) { 
            //     paused = true;
            //     switch_one = true;
            //
            ...
            //     if(current_goal.counter == 4 && distancePoint(current_goal.pose, current_pose.world_position) <= 2) {
            //         paused = false;
            //         switch_two = true;
            //
            ...
            //
            //     if(distancePoint(current_pose.world_position, end_course) < 2 && !stop_node) {
            ...
            //         ROS_INFO_STREAM("Time taken to complete the course: " << time_taken.toSec() << " seconds");
            //         ROS_INFO("==========================================");
            //
            ...
            // }
            //
            // // cout << current_pose.world_position.x << " " << current_pose.world_position.y << endl;
            ...
            if (true) {
                {
                    ...
                    if (last_goal_cell == GridCell(-1, -1)) {
                        closest_lane_cell = findClosestLaneParallel(current_map.grid, current_map.width, current_pose.map_position);
                        goal_cell = exploreFarthestParallel(valid_lane_cells, closest_lane_cell, 5);

                        int size_of_crop = static_cast<int>(distanceGrid(current_pose.map_position, goal_cell));

                        for (int i = max(0, current_pose.map_position.x - size_of_crop); i <= min(current_map.width - 1, current_pose.map_position.x + size_of_crop); i++) {
                            for (int j = max(0, current_pose.map_position.y - size_of_crop); j <= min(current_map.height - 1, current_pose.map_position.y + size_of_crop); j++) {
                        ...
                        if (valid_lane_cells.empty()) {
                            ...
                            closest_lane_cell = findClosestLaneParallel(current_map.grid, current_map.width, last_goal_cell);
                            goal_cell = exploreFarthestParallel(valid_lane_cells, closest_lane_cell, 5);
                    ...
                    goal_cell_e.x = goal_cell.x + 30*cos(current_pose.yaw);
                    goal_cell_e.y = goal_cell.y + 30*sin(current_pose.yaw);
                    ...
                    // cout << "Bot Pose: " << current_pose.map_position.x << " " << current_pose.map_position.y << endl;
                    // cout << "Closest lane cell: " << closest_lane_cell.x << " " << closest_lane_cell.y << endl;
                    // cout << "Ignored Cells: " << count_if(ignored_cells.begin(), ignored_cells.end(), [](const vector<bool>& row) { return count(row.begin(), row.end(), true); }) << endl;
                    // cout << "Valid lane cells: " << valid_lane_cells.size() << endl;
...
int main(int argc, char** argv) {
    ...
    ros::Subscriber map_sub = nh.subscribe("/local_map", 1, mapCallback);
    ros::Subscriber odom_sub = nh.subscribe("/odom", 1, odomCallback);
    ros::Subscriber global_goal_sub = nh.subscribe("/goal", 1, globalGoalCallback);
    ...
    //     ROS_INFO_STREAM("Total runtime: " << (ros::Time::now() - start_time).toSec() << " seconds");
    // }
