...
def generate_launch_description():
    ...
    declare_pointcloud_topic = DeclareLaunchArgument(
        'pointcloud_topic', default_value='/ouster/points2'
    )
    declare_imu_topic = DeclareLaunchArgument(
        'imu_topic', default_value='/ouster/imu2'
    )
    ...
    driver_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(ouster_ros_dir, 'launch', 'sensor.launch.xml')),
    ...
    zed_no_tf_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(zed_wrapper_dir, 'launch', 'zed_no_tf.launch.py'))
    )
