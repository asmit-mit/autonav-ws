...
public:
  PointCloudTransformer() {
    ...
    ROS_INFO(
        "[odom_pc] PointCloud Transformer initialized. Waiting for data..."
    );
  }
  ...
  void pointcloudCallback(const sensor_msgs::PointCloud2ConstPtr &cloud_msg) {
    ...
    try {
      ...
      transform_stamped = tf_buffer->lookupTransform(
          "odom", cloud_msg->header.frame_id, ros::Time(0), ros::Duration(1.0)
      );
      ...
      ROS_WARN("[odom_pc] Could not transform pointcloud: %s", ex.what());
...
private:
  void monitorCallback(const ros::TimerEvent &) {
    ...
    if (!received_pointcloud) {
      ROS_WARN_THROTTLE(
          5.0, "[odom_pc] No PointCloud data has been received yet from "
      ...
      if (time_since_last_pc > 1.0) {
        ROS_WARN_THROTTLE(
            5.0, "[odom_pc] No PointCloud data received for %.1f seconds",
            time_since_last_pc
        );
      }
    }
    ...
    if (!transform_available) {
      ROS_WARN_THROTTLE(
          5.0, "[odom_pc] Transform from %s to base_link not available",
      ...
      if (time_since_last_transform > 1.0) {
        ROS_WARN_THROTTLE(
            5.0, "[odom_pc] No successful transform for %.1f seconds",
